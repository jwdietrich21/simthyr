...
uses
  ...
  StdCtrls, ExtCtrls, ActnList, types, LCLType, SimThyrTypes, SimThyrGUIServices,
...
begin
  ...
  {$ENDIF}
  if i > 1 then
    imax_old := imax  {continue halted simulation}
  else
    imax_old := 0;    {new simulation}
  ...
  begin
    ...
    runcommand := True;
    Close;
    imax := imax_old + cycles(tmax, tmax_unit);
    gLoadInfo.startTime   := imax_old + cycles(lt, i1_unit);
    SetStatusBarPanel0('   ' + IntToStr(imax_old) + ':', IntToStr(imax));
