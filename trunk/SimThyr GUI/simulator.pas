...
uses
  ...
  Classes,
  {$IFDEF GUI}
  Forms, SimThyrGUIServices, SimThyrLog,
  SimThyrPlot, SimThyrPrediction, HandleNotifier, 
  {$ELSE}
  StrUtils, SimThyrCLIServices,
  {$ENDIF}
  SyncObjs, SysUtils, Math, SimThyrTypes, SimThyrResources, SimThyrBaseServices,
  Predictor, UnitConverter, ScenarioHandler;
...
type
  ...
  private
    resultMatrix: TResultMatrix;
    resultString: tResultContent;
    nmaxString, nString: string;
    T4conversionFactor, T3conversionFactor: real;
    inv_hpd, inv_imax, inv_UTRH: real;
    ultrashortFeebackGain: real;
    gainOfTSH, gainOfT4, gainOfCentralT3, gainOfPeripheralT3: real;
    procedure SignalAsRunning;
    procedure Synchro;
  public
    n, nmax: longint;
    procedure Pause;
...
function cycles(n: longint; n_text: str255): integer;
procedure InitSimulationControl;
procedure SimHypothalamus(const inv_hpd: real);
procedure SimPituitary(const gainOfTSH: real; const ultrashortFeedbackGain: real;
  memory: boolean);
...
begin
  ...
  t := 0;
  delt := 100;
  {calculation steps in seconds; e.g. 1/3 of smallest Halflife}
  i0 := 0;
end;
...
begin
  dTSH := 0.001;
  {mU/s    Inhibited production rate [calculated according to D'Angelo 1976, Okuno 1979 and Greenspan 1997]}
  {InitialValues:}
  gActiveModel.Equilibrium.TRHs := 2500 * UTRH;
  {endogenious TRH, according to Rondeel et al. 1988}
  TRH := TRH0;
  ...
  IBS := 8e-6;
  {mol/l    estimated from TBG level, corrected for intracellular accumulation [Hays et al. 1988]}
...
begin
  ...
  gH := dTSH * (DH + TRH0) * (1 + LS * T3R) * (1 + Ss * alphaS2 *
    dTSH / (betaS2 * DS + alphaS2 * dTSH)) / TRH0;
...
begin
  ...
  if circadianflag then
    ...
    circadianControl := 0;
  TRHi := gActiveModel.Equilibrium.TRH1 + THREE_FIFTH *
    gActiveModel.Equilibrium.TRHs * circadianControl;
...
begin
  ...
  alphaS := 0.4;
  {/l: from V0 = 2.5l (peptide hormone disbributing in peripheral plasma only)}
  betaS := 2.3e-4;
  {/s: from halflife of about 50 Min. [Li et al. 1995, Odell et al. 1967]}
  alphaS2 := 2.6e5;
  {/l: from V0 = 3,8µl (calculated according to Edelman 1959 and other sources)}
  betaS2 := 140;
  {/s: primary unknown kinetics, conclusions by analogy from typical receptor potentials, corresponding to halflife of 5 ms}
  dH := 47e-9;                 {mol/l: [Le Dafniet et al. 1994]: 47 nmol/l}
  dR := 0.1e-9;                {mol/l: [Vybok et al. 1994, Lazar et al., 1990]}
  GR := 1;
  {mol/s: scaled to 1, as amplification of feedback path quantified by LS}
  SS := 100;
  {l/mU: calculated from results of [Kakita et al. 1984] with Lineweaver-Burk regression}
  DS := 50;
  {mU/l: calculated from results of [Kakita et al. 1984] with Lineweaver-Burk regression}
  ...
  GD1 := 2.8e-8;
  {mol/sec: determined from reference values from SPINA I trial}
  ...
  {for calculating LS from clinical data}
  ...
  LS := MI / G3;
  {Compensation for GR; LS therefore depending from empirical data only; approximation: 1.68 e6 l/mol}
  ...
  if gStartup then
    ...
    gH := gH0;
  {restores gH to native value in order to avoid interference as gH is infered from other parameters}
  ...
  Tt22 := 3240;
  {dead time for Ultrashort feedback effect [derived from Greenspan 1997]: Tau 0 S2}
...
procedure TSimulationThread.SignalAsRunning;
// inform main thread that the simulation is not ready
begin
  simready := False;
end;

procedure TSimulationThread.Synchro;
// synchronized parts to be performed in the main thread
begin
  while n <= nmax do
  begin
    nString := IntToStr(n);
    if not haltsim then
    begin
      gResultMatrix[n - 1, i_pos] := n;
      gResultMatrix[n - 1, t_pos] := t;
      gResultMatrix[n - 1, TRH_pos] := TRH * inv_UTRH;
      gResultMatrix[n - 1, pTSH_pos] := TSHz;
      gResultMatrix[n - 1, TSH_pos] := TSH;
      gResultMatrix[n - 1, TT4_pos] := T4 * T4conversionFactor;
      gResultMatrix[n - 1, FT4_pos] := FT4 * T4conversionFactor;
      gResultMatrix[n - 1, TT3_pos] := T3p * T3conversionFactor;
      gResultMatrix[n - 1, FT3_pos] := FT3 * T3conversionFactor;
      gResultMatrix[n - 1, cT3_pos] := T3z * T3conversionFactor;
      resultString[i_pos] := nString;
      resultString[t_pos] := FormattedTime(t);
      resultString[TRH_pos] :=
        FormatFloat(gNumberFormat, gResultMatrix[n - 1, TRH_pos] *
        gParameterFactor[TRH_pos]);
      {TRH}
      resultString[pTSH_pos] :=
        FormatFloat(gNumberFormat, gResultMatrix[n - 1, pTSH_pos] *
        gParameterFactor[pTSH_pos]);
      {portal TSH}
      resultString[TSH_pos] :=
        FormatFloat(gNumberFormat, gResultMatrix[n - 1, TSH_pos] *
        gParameterFactor[TSH_pos]);
      {serum TSH}
      resultString[TT4_pos] :=
        FormatFloat(gNumberFormat, gResultMatrix[n - 1, TT4_pos] *
        gParameterFactor[TT4_pos]); {T4}
      resultString[FT4_pos] :=
        FormatFloat(gNumberFormat, gResultMatrix[n - 1, FT4_pos] *
        gParameterFactor[FT4_pos]);
      {FT4}
      resultString[TT3_pos] :=
        FormatFloat(gNumberFormat, gResultMatrix[n - 1, TT3_pos] *
        gParameterFactor[TT3_pos]);
      {T3p}
      resultString[FT3_pos] :=
        FormatFloat(gNumberFormat, gResultMatrix[n - 1, FT3_pos] *
        gParameterFactor[FT3_pos]);
      {FT3}
      resultString[cT3_pos] :=
        FormatFloat(gNumberFormat, gResultMatrix[n - 1, cT3_pos] *
        gParameterFactor[cT3_pos]);
      {T3z}
            {$IFDEF GUI}
            writeTableCells(SimThyrLogWindow.ValuesGrid, resultString);
            if n mod 30 = 0 then
              SetStatusBarPanel0(nString, nmaxString);
            if imax > 0 then
              SimThyrLogWindow.ProgressBar1.Position := trunc(100 * n / imax)
            else
              SimThyrLogWindow.ProgressBar1.Position := 100;
            application.ProcessMessages;
            {$ELSE}
      WriteTableLine(resultString);
            {$ENDIF}
      t := t + delt;
      {Hypothalamus:}
      ...
      Inc(n);
      {Load:}
      case gLoadInfo.kind of
        lkTRH:
          if n >= gLoadInfo.startTime then
            TRHe := UTRH * (200000 * alphaR * exp(
              -betaR * (t - gLoadInfo.startTime * delt)));
        lkTBG:
          if n >= gLoadInfo.startTime then
            TBG := 4.5e-7;
      end;
...
begin
  Synchronize(@SignalAsRunning);
  inv_hpd := 1 / HoursPerDay;   { optimization for speed: }
  inv_imax := 1 / imax;         { on most processors multiplications are }
...
constructor TSimulationThread.Create(CreateSuspended: boolean);
begin
...
begin
  SetLength(gResultMatrix, imax, RES_MAX_COLS);
  {$IFDEF GUI}
  SetStatusBarPanel0('   0', IntToStr(imax));
  graphready := false;
  ...
  begin
    ...
    SimThread := TSimulationThread.Create(True);
  {$IFNDEF GUI}
  //SimThread.Execute;
  //SimThread.SafeFree;
  {$ENDIF}
...
{References:  }{1. Neuber, H., "Simulation von Regelkreisen auf Personal Computern  }{  in Pascal und Fortran 77", IWT, Vaterstetten 1989  }{2. Mark, D., C. Reed, "Macintosh Pascal Programming Primer"  }{  Addison-Wesley, Reading, Ma., Mento Park, Ca., New York 3° 1993  }{3. Borenstein, Ph., J. Mattson, "THINK Pascal™ User Manual"  }{  Symantec Corporation, Cupertino, Ca. 1990   }{4. Dahl, G. E., N. P. Evans, L. A. Thrun, f. J. Karsch (1994) "A Central   }{  Negative  Feedback Action of Thyroid Hormones on Thyrotropin-  }{  Releasing Hormone Secretion", Endocrinology 195: 2392-7  }{5. Samuels, M. H., P. Henry, M. Luther, E. C. Ridgway (1993) "Pulsatile  }{  TSH Secretion during 48-Hour Continuous TRH Infusions", Thyroid  }{  3: 201-6  }{6. Li, G., B. Liu, Y. Liu (1995) "A dynamical model of the pulsatile  }{  secretion of the hypothalamo-pituitary-thyroid axis", BioSystems  }{  35: 83-92  }{7. Greenspan, F. S. "The Thyroid Gland" in: Greenspan, F. S.,  }{  Strewler, G. J., "Basic and Clinical Endocrinology",  }{  Appleton & Lange, Stamford, CT, 1997  }{8. Visser, T. J., Kaplan, M. M., Leonard, J. L., Larsen, P. R. (1983)  }{  "Evidence for Two Pathways of Iodothyronine 5'-Deiodination in Rat  }{  Pituitary That Differ in Kinetics, Propylthiouracil Sensitivity and  }{  Respone to Hypothyroidism", J. Clin. Invest. 71: 992-1002  }{9. Lee TW; Anderson LA; Eidne KA; Milligan G (1995)   }{  "Comparison of the signalling properties of the long and short   }{  isoforms of the rat thyrotropin - releasing - hormone receptor  }{   following expression in rat 1 fibroblasts", Biochem J 310: 291-8  }{10. Karhapaa L; Tornquist K (1995) "Caffeine inhibits the binding of  }{  thyrotropin-releasing hormone in GH4C1 pituitary cells."  }{  Biochem Biophys Res Commun 210(3):726-32  }{11. Dumont, J. E., Vassart, G. "Thyroid Regulation" in De. Groot  }{  "Endocrinology", WB Saunders 1995  }{12. Jackson. I. M. D., "Thyrotropin Releasing Factor" in Adelman, G.  }{  "Encyclopedia of Neuroscience", Volume II, Birkhäuser, Boston,  }{  Basel, Stuttgart, 1987  }{13. Okuno, A., Taguchi, T., Takimoto, M. (1979) "Kinetic Analysis of   }{  Plasma TSH Dynamics after TRH Stimulation" Horm. Metab. Res. 11:  }{  293-5  }{14. D'Angelo, S. A., Paul, D. H., Wall, N. R., Lombardi, D. M. (1976)  }{  "Pituitary Thyrotropin (TSH) Rebound Phenomenon and Kinetics of  }{  Secretion in the Goitrous Rat: Differential Effects of Thyroxine on  }{  Synthesis and Release of TSH" Endocrinology 99: 935-43  }{15. Rondeel, J. M. M., de Greef, W. J., van der Schoot, P., Karels, B.,  }{  Klootwijk, W., Visser, T. J. (1988) "Effect of Thyroid Status and  }{  Paraventricular Area Lesions on the Release of Thyrotropin-  }{  Releasing Hormone and Catecholamines into Hypophysial Portal  }{  Blood" Endocrinology 123:523-7  }{16. Oppenheimer, J. H., G. Bernstein, J. Hasen (1967) "Estimation of  }{  Rapidly Exchangeable Cellular Thyroxine from the Plasma  }{  Disappearance Curves of Simultaneously Administered Thyroxine-  }{  131I and Albumin-125I" J. Clin. Invest 46: 762-77  }{17. Edelman, I. S., J. Leibman (1959) "Anatomy of Body Water and  }{  Electrolytes", Am. J. Med. 27, 256-77  }{18. Kakita, T., Laborde, N. P., Odell, W. D. (1984) "Autoregulatory  }{  Control of Thyrotrophin in Rabbits", Endocrinology 114: 2301-5  }{19. Dietrich, J. W., Tesche, A., Pickardt, C. R., Mitzdorf, U. (2004) }{  "Thyrotropic Feedback Control: Evidence for an Additional Ultrashort }{  Feedback Loop from Fractal Analysis", Cybernetics and Systems 35: }{  315-31 }{20. Dietrich JW, Stachon A, Antic B, Klein HH, Hering S. (2008) }{  "The AQUA-FONTIS study: protocol of a multidisciplinary, cross-sectional }{  and prospective longitudinal study for developing standardized diagnostics }{  and classification of non-thyroidal illness syndrome", BMC Endocr Disord. }{  8:13. doi: 10.1186/1472-6823-8-13. PMID 18851740 }{21. Dietrich JW, Landgrafe G, Fotiadou EH. (2012) "TSH and Thyrotropic }{  Agonists: Key Actors in Thyroid Homeostasis", J Thyroid Res. 2012:351864. }{  doi: 10.1155/2012/351864. PMID 23365787 }{22. Hoermann R, Midgley JE, Larisch R, Dietrich JW. (2013) "Is pituitary }{  TSH an adequate measure of thyroid hormone-controlled homoeostasis during }{  thyroxine treatment?", Eur J Endocrinol. 168(2):271-80. }{  doi: 10.1530/EJE-12-0819. PMID 23184912. }{23. Midgley JE, Hoermann R, Larisch R, Dietrich JW. (2013) "Physiological }{  states and functional relation between thyrotropin and free thyroxine in }{  thyroid health and disease: in vivo and in silico data suggest a }{  hierarchical model", J Clin Pathol. 66(4):335-42. }{  doi: 10.1136/jclinpath-2012-201213. PMID 23423518. }{24. Hoermann R, Midgley JE, Larisch R, Dietrich JW (2015). }{  Integration of Peripheral and Glandular Regulation of }{  Triiodothyronine Production by Thyrotropin in Untreated and }{  Thyroxine-Treated Subjects. Hormone and Metabolic Research, }{  47(9):674-80. PMID: 25750078 doi: 10.1055/s-0034-1398616. }{25. Midgley JE, Larisch R, Dietrich JW, Hoermann R. (2015). }{  Variation in the biochemical response to l-thyroxine therapy and }{  relationship with peripheral thyroid hormone conversion efficiency. }{  Endocr Connect. 2015 Dec;4(4):196-205. doi: 10.1530/EC-150056. }{  PMID 26335522. }{26. Dietrich JW, Midgley JE, Larisch R, Hoermann R. (2015). }{  Of rats and men: thyroid homeostasis in rodents and human beings. }{  Lancet Diabetes Endocrinol. 2015 Dec;3(12):932-3. }{  doi: 10.1016/S2213-8587(15)00421-0. PMID 26590684. }{27. Hoermann R, Midgley JE, Larisch R, Dietrich JW. (2015). }{  Homeostatic Control of the Thyroid-Pituitary Axis: Perspectives for }{  Diagnosis and Treatment. Front Endocrinol (Lausanne). 2015 }{  Nov 20;6:177. doi: 10.3389/fendo.2015.00177. PMID 26635726. }{28. Dietrich JW, Landgrafe-Mende G, Wiora E, Chatzitomaris A, }{  Klein HH, Midgley JE, Hoermann R. (2016). Calculated Parameters }{  of Thyroid Homeostasis: Emerging Tools for Differential Diagnosis }{  and Clinical Research. Front Endocrinol (Lausanne). 2016 Jun 9;7: }{  57. doi: 10.3389/fendo.2016.00057. PMID 27375554. }
