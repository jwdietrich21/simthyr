...
uses
  ...
  ExtCtrls, StdActns, LCLType, Menus, ActnList, ComCtrls, LCLIntf, LCLProc,
  StdCtrls, LCLVersion, ExtDlgs, SimThyrTypes, SimThyrResources, SimOptions,
  SimThyrBaseServices, SimThyrGUIServices, LaunchDialog, ShowIPS, Simulator,
  Printers, SimThyrLog, SimThyrPlot, AboutDialog, ShowAboutModel,
  StructureParameters, Predictor, SimThyrPrediction, Sensitivityanalysis,
  TWSensitivityanalysis, tornado, Equilibriumdiagram, help, ScenarioHandler,
  HandlePreferences, HandleNotifier, EnvironmentInfo, MIRIAMForm;
...
begin
  ...
  {$ENDIF}
    ...
    {if assigned(SimThread) then         { #todo -oJWD -cMultithreading : Reenable after debugging }
      SimThread.SafeFree;
    SimThread := nil;  }
  end
  else
...
begin
  {if haltsim then
    SimThread.Restart             { #todo -oJWD -cMultithreading : Reenable after debugging }
  else
    SimThread.Pause; }
end;
...
begin
  ...
  graphready := False;
  imax := 0;
  {if SimThread <> nil then             { #todo -oJWD -cMultithreading : Reenable after debugging }
  begin
