...
uses
  ...
  StdCtrls, ExtCtrls, SimThyrTypes, SimThyrGUIServices, EnvironmentInfo;
...
begin
  ...
  TRHEdit.Text := FloatToStrF(gActiveModel.Equilibrium.TRHs / UTRH, ffGeneral, 5, 2);
  {$IFDEF LCLCocoa}
...
begin
  ...
  gActiveModel.Equilibrium.TRHs := StrToFloat(TRHEdit.Text) * UTRH;
  Close;
  {$IFDEF LCLCocoa}
  if parentForm = SimulationSettings then
    SimulationSettings.ShowModal
