...
{InitialValues:}
  gActiveModel.Equilibrium.TRHs := 2500 * UTRH; {endogenious TRH, according to Rondeel et al. 1988}
  TRH := TRH0;
 ...
 begin
   ...
   if circadianflag then
     ...
     circadianControl := 0;
   TRHi := gActiveModel.Equilibrium.TRH1 + THREE_FIFTH * gActiveModel.Equilibrium.TRHs * circadianControl;
...
{Load:}
            case gLoadInfo.kind of
              lkTRH:
                if i >= gLoadInfo.startTime then
                  TRHe := UTRH * (200000 * alphaR * exp(-betaR * (t - gLoadInfo.startTime * delt)));
              lkTBG:
                if i >= gLoadInfo.startTime then
                  TBG := 4.5e-7;
            end;
